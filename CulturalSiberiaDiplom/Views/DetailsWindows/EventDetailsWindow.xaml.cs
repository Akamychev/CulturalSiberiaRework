...
public partial class EventDetailsWindow : Window
{
    ...
    {
        InitializeComponent();
        DataContext = new EventDetailsViewModel(@event, Service.GetDbContext(), CurrentUser.SelectedUser, this);
    }
    ...
    private void Image_Drop(object sender, DragEventArgs e)
    {
        ...
        if (ImageService.IsImageFile(path))
        {
            ...
            if (DataContext is EventDetailsViewModel vm)
            {
                vm.ImageBytes = data;
    ...
    {
        if (DataContext is EventDetailsViewModel vm && vm.IsInEditMode)
